...
exports.updateZeusPreferences = exports.getZeusPreferences = exports.executeZeusAction = exports.processZeusQuery = void 0;
const admin = __importStar(require("firebase-admin"));
const functions = __importStar(require("firebase-functions"));
// Exportar funciones de Zeus
exports.processZeusQuery = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!query) {
            throw new functions.https.HttpsError('invalid-argument', 'Se requiere una consulta');
        }
...
exports.executeZeusAction = functions.https.onCall(async (data, context) => {
    // Verificar autenticación
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Se requiere autenticación para esta operación');
    }
    try {
        ...
        // Validar datos requeridos
        if (!action) {
            throw new functions.https.HttpsError('invalid-argument', 'Se requiere una acción');
        }
...
exports.getZeusPreferences = functions.https.onCall(async (data, context) => {
    // Verificar autenticación
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Se requiere autenticación para esta operación');
    }
...
exports.updateZeusPreferences = functions.https.onCall(async (data, context) => {
    // Verificar autenticación
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Se requiere autenticación para esta operación');
    }
    try {
        ...
        // Validar datos requeridos
        if (!preferences) {
            throw new functions.https.HttpsError('invalid-argument', 'Se requieren preferencias');
        }
        // Actualizar preferencias
        await admin.firestore().collection('zeus_preferences').doc(userId).update(Object.assign(Object.assign({}, preferences), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
        return {
            success: true,
...
function generateZeusResponse(query, userContext) {
    ...
    const artistNames = ((_a = userContext.artists) === null || _a === void 0 ? void 0 : _a.map(artist => artist.name).join(', ')) || "tus artistas";
