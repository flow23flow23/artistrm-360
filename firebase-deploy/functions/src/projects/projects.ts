...
export const createProject = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (!name || !type) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Se requieren nombre y tipo de proyecto'
      );
    }
...
export const addProjectMember = functions.https.onCall(async (data, context) => {
  // Verificar autenticación
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
  ...
  try {
    ...
    if (!projectId || !memberEmail) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Se requieren ID del proyecto y email del miembro'
      );
    }
